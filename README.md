...
按顺序执行各阶段。`mop-idea`（轻量）、`mop-req`（完整）、`mop-import`（从已有项目导入）三者均可生成 PRD，选其一即可。当需求、设计或技术方案发生变更时，运行 `mop-sync` 同步所有下游文档。`mop-sync` 也会在其他 skill 结束时自动触发（需用户确认）。
...
| `/mop-idea` | 交互式想法梳理（轻量版），通过「聊想法 → 引导需求 → 整理排序」三步将模糊想法转化为 PRD（`docs/prd.md`）。不涉及商业模式和用户场景，专注功能需求梳理。可选阶段：竞品分析、头脑风暴、双视角需求评审 |
| `/mop-import` | 从已有项目代码或文档逆向生成 PRD（`docs/prd.md`）。自动分析项目结构、代码、README 等材料，提取功能模块和用户角色，与用户确认后补全商业模式、成功指标等无法从代码推断的信息 |
| `/mop-req` | 交互式需求收集（完整版），通过多轮对话输出 PRD（`docs/prd.md`）。根据产品定位自动切换商业模式/构建者模式调整提问深度。可选阶段：竞品分析（web search）、头脑风暴、前提挑战、双视角需求评审 |
